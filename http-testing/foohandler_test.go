...
// ? test with response recording (allows to test programatically)
func TestHandleGetFooRR(t *testing.T) {
	...
	if string(b) != expected {
		t.Errorf("expected %s but we go %s", expected, string(b))
	}
}
...
func TestHandleGetFoo(t *testing.T) {
	...
	if resp.StatusCode != http.StatusOK {
		t.Errorf("excected 200 but got %d", resp.StatusCode)
	}
